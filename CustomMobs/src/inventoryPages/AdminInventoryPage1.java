...
public class AdminInventoryPage1 implements IPage{
	...
	private void fill() {
		...
		try {
			...
			for(int i = 0;i<9;i++) {this.inv.setItem(i, main.Main.armors.get(helmets.get(i)).clone());}
			for(int i = 9;i<18;i++) {this.inv.setItem(i, main.Main.armors.get(chestPlates.get(i-9)).clone());}
			for(int i = 18;i<27;i++) {this.inv.setItem(i, main.Main.armors.get(leggings.get(i-18)).clone());}
			for(int i = 27;i<36;i++) {this.inv.setItem(i, main.Main.armors.get(boots.get(i-27)).clone());}
